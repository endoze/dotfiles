{ config, options, pkgs, lib, sourceRoot, userConfig, ... }:
...
in
{
  config = lib.mkMerge ([
    ...
    xdg.configFile."weechat/sec.conf".source = config.lib.file.mkOutOfStoreSymlink config.sops.templates."sec.conf".path;
  });
}
