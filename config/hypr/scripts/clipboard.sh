...
case "$active_class" in
  ...
  firefox|Claude|discord|Slack|r2modman|steam|google-chrome|chromium)
    case "$action" in
      copy)  hyprctl dispatch 'hl.dsp.send_shortcut({ mods = "CTRL", key = "c", window = "activewindow" })' ;;
      paste) hyprctl dispatch 'hl.dsp.send_shortcut({ mods = "CTRL", key = "v", window = "activewindow" })' ;;
